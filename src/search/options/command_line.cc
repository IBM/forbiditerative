...
static void predefine_symmetries(const string &arg, Registry &registry,
                                 Predefinitions &predefinitions, bool dry_run) {
    pair<string, string> predefinition = split(arg);
    OptionParser parser(predefinition.second, registry, predefinitions, dry_run);
    predefinitions.predefine(predefinition.first,
                             parser.start_parsing<shared_ptr<Group>>());
}
    ...
    */
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        if (arg == "--evaluator") {
            ...
            ++i;
            predefine_symmetries(sanitize_string(args[i]), registry, predefinitions,
                                 dry_run);
        } else if (arg == "--search") {
            if (is_last)

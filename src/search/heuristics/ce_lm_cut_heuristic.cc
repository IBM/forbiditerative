...
int CELandmarkCutHeuristic::compute_heuristic(const GlobalState &global_state) {
    State state = convert_global_state(global_state);
    return compute_heuristic(state);
}

int CELandmarkCutHeuristic::compute_heuristic(const State &state) {
    int total_cost = 0;
...
static Heuristic *_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return new CELandmarkCutHeuristic(opts);
}

static Plugin<Heuristic> _plugin("celmcut", _parse);
}

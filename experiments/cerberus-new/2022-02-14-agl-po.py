...
def main(revisions=None):
    ...
    configs.add(IssueConfig('cerberus2022-argmax', [
        "--if-unit-cost",
        "--evaluator",
        ...
        "--evaluator", 
        "hn=novelty(eval=hrb, type=separate_both, pref=true, cutoff_type=argmax)",        
    ...
    configs.add(IssueConfig('cerberus2022-co1', [
        "--if-unit-cost",
        "--evaluator",
        "hlm=lmcount(lm_reasonable_orders_hps(lm_rhw()),pref=true)",
        "--evaluator", 
        "hrb=RB(dag=from_coloring, extract_plan=true)",
        "--evaluator", 
        "hn=novelty(eval=hrb, type=separate_both, pref=true, cutoff_type=all_ordered, cutoff_bound=1)",        
        "--search", """lazy(open=alt([tiebreaking([hn, hrb]), single(hn,pref_only=true), single(hlm), single(hlm,pref_only=true)], boost=1000),preferred=[hrb,hlm])""",
        "--if-non-unit-cost",
        "--evaluator",
        "hlm=lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=true)",

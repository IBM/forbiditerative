...
def add_exp(planner_name, args, alg, exp_k):
    ...
    for task in suites.build_suite(BENCHMARKS_DIR, SUITE):
        ...
        run.add_command(
            'run-planner',
            [planner_name,
            "--planner",  args["planner"], "--domain", '{domain}', "--problem", '{problem}', "--number-of-plans", exp_k, "--symmetries", "--use-local-folder", "--clean-local-folder", "--suppress-planners-output", "--overall-time-limit", 1800],
            time_limit=1800,
        ...
        run.set_property('algorithm', "FI-%s-%s" % (alg, exp_k) )
...
planners = { "topk": {"planner": "topk"},
"topk-uq" : {"planner": "topk_via_unordered_topq" }
}

for k in [1000]:
    for alg in planners:
        planner_name = os.path.join(get_planner_dir(), "plan.py")
        add_exp(planner_name, planners[alg], alg, k)

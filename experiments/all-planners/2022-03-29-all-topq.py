...
def add_exp(planner_name, args, alg, exp_q):
...
planners = { 
    "topuq": {"planner": "unordered_topq"},
    "topq-k" : {"planner": "topq_via_topk"},
    "topq-uq" : {"planner": "topq_via_unordered_topq"}
}

for q in [1.0, 1.1]:
    for alg in planners:
        planner_name = os.path.join(get_planner_dir(), "plan.py")
        add_exp(planner_name, planners[alg], alg, q)

...
def main(revisions=None):
    ...
    suite = ['briefcaseworld', 'cavediving-14-adl', 'citycar-sat14-adl', 'fsc-blocks', 'fsc-grid-a1', 'fsc-grid-a2', 'fsc-grid-r', 'fsc-hall', 'fsc-visualmarker', 'gedp-ds2ndp', 'miconic-simpleadl', 't0-adder', 't0-coins', 't0-comm', 't0-grid-dispose', 't0-grid-push', 't0-grid-trash', 't0-sortnet', 't0-sortnet-alt', 't0-uts', "agricola-sat18", "caldera-sat18", "caldera-split-sat18", "data-network-sat18", "flashfill-sat18", "nurikabe-sat18", "organic-synthesis-sat18", "organic-synthesis-split-sat18", "settlers-sat18", "snake-sat18", "spider-sat18", "termes-sat18"]
    ...
    configs = {
        IssueConfig('rb-multiply-out-ce', ['--heuristic', 'hrb=RB(dag=from_coloring, extract_plan=true, transform=multiply_out_conditional_effects())', '--search', 'lazy_greedy([hrb],reopen_closed=false)'],
    ...
    OTHER_REV = ''
    exp.add_fetcher('data/2018-06-22-lifted-stabinit-stabgoal-order-mk-eval',filter_algorithm=['{}-{}'.format(OTHER_REV, x) for x in algorithm_nicks])

...
function extend_plans_complete {
    ...
    if [ "$NUM_PLANS" -gt $num_found_plans ]; then
        ...
        for ((i=1; i<=$num_found_plans; i++)); do
            ...
            (cd $PLANSDIR && 
                export PYTHONPATH=$PWD && python $SOURCE/forbiditerative/fast-downward.py $domain $problem --internal-previous-portfolio-plans $found_so_far --search $SEARCH_PARAMS)
